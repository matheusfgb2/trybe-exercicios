...
//bonus 4: Depois, faça uma pirâmide com n asteriscos de base

// bonus 3: outra maneira
let n = 30;
let symbol = '*';
let space = ' ';
let inputLine = '';

for (let indexMain = 1; indexMain <= n; indexMain += 1) {
  for (let index = 0; index < n; index += 1) {
    if (inputLine.length < n - indexMain) {
      inputLine += space;
    } else {
      inputLine += symbol;
    }
